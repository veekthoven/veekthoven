...
export default {
	...
	async asyncData ({ error, $content, params }) {
		...
		const articles = await $content().where({ 'tags': { $contains: params.tag } }).limit(10).sortBy('postedAt', 'desc').fetch()
		return {
			articles
		}
	},
	...
	head () {
	    return {
	      title: `Articles tagged ${this.$route.params.tag} - Victor Abbah Nkoms`,
	      meta: [
